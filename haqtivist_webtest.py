...
# ============ BANNER HOLOGRAM HAQTIVIST ============
def print_hologram_banner():
    hologram_colors = [Fore.CYAN, Fore.MAGENTA, Fore.BLUE, Fore.LIGHTCYAN_EX, Fore.LIGHTMAGENTA_EX]
...
╚══════════════════════════════════════════════════════════════╝
    ...
    for line in banner_text.split('\n'):
        color = random.choice(hologram_colors)
        print(color + line + Style.RESET_ALL)
        time.sleep(0.02)  # Efek animasi hologram
...
def live_stats_display(threads_active, elapsed, rps):
    ...
    sys.stdout.write(f"{Fore.CYAN}⚡ Active: {threads_active} | "
                     f"{Fore.GREEN}✓ Success: {stats['success']} | "
                     f"{Fore.RED}✗ Fail: {stats['fail']} | "
                     f"{Fore.YELLOW}📊 RPS: {rps:.1f} | "
...
def attack_mode(target, port, endpoint, duration, threads, method="GET", delay=0):
    ...
    print(f"\n{Fore.CYAN}[+] Memulai test mode ke {target}:{port}{endpoint}")
    print(f"{Fore.YELLOW}[!] Threads: {threads} | Duration: {duration}s | Method: {method} | Delay: {delay}s")
...
def show_final_report(duration):
    """Laporan akhir dengan analisis lengkap"""
    elapsed = time.time() - duration
    
    print(f"\n\n{Fore.CYAN}{'='*70}")
    print(f"{Fore.MAGENTA}📊 HASIL AKHIR HAQTIVIST HTTP TESTER")
    ...
    print(f"{Fore.WHITE}\n⚡ PERFORMANCE:")
    print(f"  {Fore.CYAN}🕒 Durasi: {elapsed:.2f} detik")
    print(f"  {Fore.CYAN}🚀 Rata-rata RPS: {stats['total_requests']/elapsed:.1f} req/detik")
    print(f"  {Fore.CYAN}📊 Peak RPS: {max(stats['response_times']) if stats['response_times'] else 0:.0f}")
    ...
    if stats['response_times']:
        ...
        print(f"  {Fore.CYAN}⏱️  Response Time (avg): {avg_response:.1f}ms")
        print(f"  {Fore.CYAN}⏱️  Response Time (min/max): {min_response:.1f}/{max_response:.1f}ms")
    
    print(f"{Fore.WHITE}\n💾 BANDWIDTH:")
    print(f"  {Fore.CYAN}📤 Total Transfer: {stats['bytes_transferred']/1024/1024:.2f} MB")
    print(f"  {Fore.CYAN}📤 Rata-rata Transfer: {stats['bytes_transferred']/1024/elapsed:.1f} KB/s")
...
if __name__ == "__main__":
    ...
    show_final_report(time.time() - (duration if 'start_time' in dir() else 0))
